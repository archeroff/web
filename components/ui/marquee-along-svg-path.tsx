...
const MarqueeItem = ({
  ...
  const cssVariables: Record<string, MotionValue<number | string>> = {}
  for (const { property, from, to } of cssVariableInterpolation) {
    // The upstream component calls this hook dynamically per CSS variable.
    // In practice the prop defaults to an empty array, so the call order is
    // stable (zero iterations). Keep it to preserve the upstream API.
    // oxlint-disable-next-line react-hooks/rules-of-hooks
    cssVariables[property] = useTransform(
